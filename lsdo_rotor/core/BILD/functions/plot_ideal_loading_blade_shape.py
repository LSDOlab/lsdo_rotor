...
def plot_ideal_loading_blade_shape(sim, sim2=None):
    ...
    # axs[0].plot( radius.flatten(),local_chord_opt.flatten()/2, marker = '*', color = 'navy', label = 'chord length with root correction')
    # axs[0].plot( radius.flatten(),local_chord_opt.flatten()/-2, marker = '*', color = 'navy')
    ...
    # axs[1].plot( radius.flatten(),local_twist_angle_opt.flatten()* 180 /np.pi, marker = 'o', color = 'navy')
