...
def get_plot_sweeps_vs_r(airfoil, rotor_files, RPM, rotor_diameter, V_inf, num_blades, altitude, n_radial, optimized_pitch_cp, optimized_chord_cp, pitch_cp, chord_cp):
    ...
    C_T_mat = thrust / 1.2 / prob['rotational_speed']**2 / (2 * prob['rotor_radius'])**4
    P = 2 * np.pi * prob['rotational_speed'] * torque
    C_P_mat = P / 1.2 / prob['rotational_speed']**3 / (2 * prob['rotor_radius'])**5
    eta_total_mat = J * C_T_mat / C_P_mat
    ...
    fig.suptitle('Rotor Parameter Sweeps vs. Radius with Blade Element Momentum Theory' + '\n' + 'RPM = {}, V = {}'.format(RPM,V_inf),fontsize=23)
    
    axs[0,0].plot(radius,  eta_total_mat, label = r'$\eta$ ')
    axs[0,0].set_xlim(left = 0)
    axs[0,0].set_ylabel(r'Efficiency $\eta$',fontsize=19)
    axs[0,0].set_xlabel('Radius (m)',fontsize=19)
    # axs[0].legend(fontsize=19)
    
    axs[1,0].plot(radius, Re, label = 'Re')
    axs[1,0].set_ylabel('Reynolds number',fontsize=19)
    axs[1,0].set_xlabel('Radius (m)',fontsize=19)
    
    
    axs[0,1].plot(radius, C_T_mat, label = r'$C_{T}$ ')
    axs[0,1].set_xlim(left=0)
    axs[0,1].set_ylabel(r'Thrust Coefficient $C_T$',fontsize=19)
    axs[0,1].set_xlabel('Radius (m)',fontsize=19)
    # axs[0,1].legend(fontsize=19)
    
    axs[1,1].plot(radius, C_P_mat, label = r'$C_{P}$ ')
    axs[1,1].set_xlim(left=0)
    axs[1,1].set_ylabel(r'Power Coefficient $C_P$',fontsize=19)
    axs[1,1].set_xlabel('Radius (m)',fontsize=19)
    # axs[1,1].legend(fontsize=19)
